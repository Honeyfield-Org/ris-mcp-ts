...
/**
 * Court/jurisdiction types for case law searches in RIS — see
 * {@link JUDIKATUR_GERICHTSBARKEITEN} for the values and what they select.
 */
export const JudikaturGerichtSchema = z.enum(JUDIKATUR_GERICHTSBARKEITEN);
export type JudikaturGericht = z.infer<typeof JudikaturGerichtSchema>;

/**
 * Document type filter for Judikatur searches — see
 * {@link JUDIKATUR_DOKUMENTTYPEN} for what each value searches.
 */
export const JudikaturDokumenttypSchema = z.enum(JUDIKATUR_DOKUMENTTYPEN);
export type JudikaturDokumenttyp = z.infer<typeof JudikaturDokumenttypSchema>;

/**
 * Broad legal area filter (Justiz only) — see {@link JUDIKATUR_RECHTSGEBIETE}.
 */
export const JudikaturRechtsgebietSchema = z.enum(JUDIKATUR_RECHTSGEBIETE);
